...
- [ ] 2.1 Implement parsing and normalization for one `NdM` expression with an optional signed modifier, and verify table-driven tests accept documented syntax and reject unsupported notation.
- [ ] 2.2 Implement pre-execution validation for expression length, dice count, sides, modifier, and seed bounds, and verify every limit produces the specified HTTP error code when integrated.
...
- [ ] 3.1 Add `GET /health` with the specified ready response, and verify an endpoint test receives HTTP 200 and `{"status":"ok"}`.
